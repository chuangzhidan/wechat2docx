  ...
  <div class="panel active" id="panel-url">
    ...
    <div class="input-row">
      <input id="url-input" type="url" required
        placeholder="https://mp.weixin.qq.com/s/... �� https://xueqiu.com/..."
        autocomplete="off">
    ...
    <p class="hint">支持微信公众号 & CSDN 博客 · 自动保留图片 · ���球需��览器���证</p>
    <div id="msg-url"></div>
...
urlBtn.addEventListener('click', async () => {
  ...
  try {
    const resp = await fetch('/convert', {
      ...
      body: 'url=' + encodeURIComponent(url) + '&fmt=' + selectedFmt,
    });
    clearInterval(timer);
...
async function _checkClipboard() {
  try {
    ...
    const isArticle = text.includes('mp.weixin.qq.com') || text.includes('blog.csdn.net') || text.includes('xueqiu.com');
...
def convert():
    ...
    is_xueqiu = "xueqiu.com" in url
    if not (is_wechat or is_csdn or is_xueqiu):
        return jsonify({"error": "请输入有��的文���链接（支��微信公��号、CSDN、雪球）"}), 400
    ...
    try:
        ...
        if fmt == "md":
            ...
            tmp_docx.close()
            build_docx(soup, meta, tmp_docx.name)
