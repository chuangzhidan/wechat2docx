...
def _is_emoji_cp(cp: int) -> bool:
    ...
    return (
        0x2600 <= cp <= 0x27BF or    # Misc Symbols, Technical, Dingbats
...
def _split_text_by_emoji(text: str):
    ...
    segments = []
    cur_seg = text[0]
    cur_is_emoji = _is_emoji_cp(ord(text[0]))
    for c in text[1:]:
        c_is_emoji = _is_emoji_cp(ord(c))
...
def _add_hyperlink(p, text: str, url: str, color=None):
    ...
    # 超链接样式：蓝色下划线 + Times New Roman 字体
        ...
        for child in node.children:
            if isinstance(child, NavigableString):
                ...
                if text.strip():
                    ...
                    if not cleaned:
                        cleaned = text.strip()
                    # 按 emoji/非 emoji 分段，emoji 段不设字体（让 Word 自动 fallback）
...
def _set_para_size(para, pt, align=None):
    ...
    # 各 run 字号 + 字体（Times New Roman；emoji run 不设字体让 Word 自动 fallback）
    for run in para.runs:
        run.font.size = Pt(pt)
        # 跳过纯 emoji run（强设字体会导致 Word 无法渲染彩色 emoji）
        if run.text and not all(_is_emoji_cp(ord(c)) or c.isspace() for c in run.text):
            run.font.name = "Times New Roman"
    # 段间距：段前段后0，单倍行距
        ...
        if elem.name == "table":
            ...
            if outer_formulas or inline_mjx:
                ...
                if elem.name in ("h1", "h2"):
                    ...
                    p = _add_rich_para(doc, elem, 10.5, css_colors=css_colors, img_cache=img_cache)
