...
def _add_hyperlink(p, text: str, url: str, color=None):
    ...
    t = OxmlElement('w:t')
    import re as _re
    t.text = _re.sub('\ufe0f(?!\u20e3)', '', text).replace('\ufe0e', '')
        ...
        for child in node.children:
            if isinstance(child, NavigableString):
                ...
                if text.strip():
                    ...
                    if not cleaned:
                        cleaned = text.strip()
                    # 剥离 standalone U+FE0F（emoji 展示变体选择符），但保留 keycap 序列中的 FE0F
                    # keycap: digit + U+FE0F + U+20E3（如 1️⃣），去掉 FE0F 后 U+20E3 单独渲染为 □
                    # 负向前瞻：FE0F 后面不跟 U+20E3 才剥离；FE0E（文本展示）始终剥离
                    import re as _re
                    cleaned = _re.sub('\ufe0f(?!\u20e3)', '', cleaned).replace('\ufe0e', '')
                    # 按 emoji/非 emoji 分段
...
def _set_para_size(para, pt, align=None):
    ...
    # 会被 _is_emoji_cp 误判为非 emoji，导致 Times New Roman 覆盖 Segoe UI Emoji
    for run in para.runs:
        run.font.size = Pt(pt)
        if not run.text:
            continue
